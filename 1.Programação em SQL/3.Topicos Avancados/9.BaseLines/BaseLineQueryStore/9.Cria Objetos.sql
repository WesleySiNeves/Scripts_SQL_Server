SELECT  TOP (20) QSQT.query_sql_text,
       CAST(QSP.query_plan AS XML) Plan_QUery,
       QSRS.avg_duration
  FROM sys.query_store_plan AS QSP
       INNER JOIN sys.query_store_query AS Q ON Q.query_id = QSP.query_id
       INNER JOIN sys.query_store_query_text AS QSQT ON QSQT.query_text_id = Q.query_text_id
       INNER JOIN sys.query_store_runtime_stats AS QSRS ON QSRS.plan_id = QSP.plan_id
 ORDER BY
    QSRS.avg_duration DESC;
GO

CREATE OR ALTER FUNCTION HealthCheck.ufnGetTopQuerysCountExecution
(
    @TopCount SMALLINT
)
RETURNS TABLE
...
SELECT q.query_id,
 ...
 WHERE
    qt.query_sql_text NOT LIKE '%sys.%'
 GROUP BY
    q.query_id,
    qt.query_text_id,
    qt.query_sql_text
 ORDER BY
    ...
    avg_duration DESC OFFSET 0 ROW FETCH NEXT @TopCount ROW ONLY;
GO

CREATE OR ALTER FUNCTION HealthCheck.ufnGetTopQuerys
(
    @TopCount SMALLINT,
    @order    VARCHAR(10) = 'TimeElepsed'
)
RETURNS TABLE
AS RETURN
...
WITH AggregatedDurationLastHour
    ...
    (
        SELECT q.query_id,
         ...
         WHERE
            rsi.start_time >= CAST(GETDATE() AS DATE)
    ...
    AS
    (
        SELECT AggregatedDurationLastHour.query_id,
               ...
               ROW_NUMBER() OVER (ORDER BY
                                      AggregatedDurationLastHour.total_duration DESC,
                                      AggregatedDurationLastHour.query_id
                                 ) AS RN
    ...
    (
        SELECT qt.query_text_id,
          ...
          FROM OrderedDuration od
               JOIN sys.query_store_query AS q ON q.query_id = od.query_id
