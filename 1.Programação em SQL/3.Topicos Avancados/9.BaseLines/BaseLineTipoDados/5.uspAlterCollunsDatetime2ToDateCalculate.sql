...
BEGIN
    ...
    --SELECT @IdRegistro = 1103,
    --       @SchemaName = 'Estagio',
    --       @TableName = 'ContratosEstagio',
    --       @Column = 'DataAtualizacao',
    --       @DataTypeAtual = 'datetime2';
    ...
    WHERE C.is_computed = 0
          ...
          AND NOT EXISTS (
                         ...
                         WHERE CC.object_id = T.object_id
                               AND PATINDEX(
                                               CONCAT('%', C.name COLLATE DATABASE_DEFAULT, '%'),
    ...
    SET @Sql += CONCAT(
			  ...
			  SELECT ',
               ...
               D.Indexable,
               D.Rows,
               CAST(1 AS BIT) as PossoAlterar,',
                          '',
                          @aspas_simples,
                          '',
                          @aspas_simples,
                          ' As DatabasesQuePossueValorHora ',
                          ',',
                          @aspas_simples,
                          @aspas_simples,
                          ' AS DatabasesQueNaoPossueValorHora,',
                          @aspas_simples,
                          DB_NAME(),
                          @aspas_simples,
                          ' AS DatabasesTableVazio',
                          ' FROM #Dados AS D
         WHERE
            D.Rows = 0 '
                      );
    ...
    SET @Sql += CONCAT('BEGIN', SPACE(1));
    SET @Sql += CONCAT(' IF(EXISTS ( SELECT CAST(X.', @Column, ' AS TIME) FROM ', @SchemaName, '.', @TableName, ' AS X');
    SET @Sql += CONCAT(' WHERE CAST(X.', @Column, ' AS TIME) <> ''00:00:00.0000000'' ))');
    SET @Sql += CONCAT(
                          ' BEGIN INSERT INTO #Retorno
										(Id,
									    SchemaName,
										TableName,
										Coluna,
