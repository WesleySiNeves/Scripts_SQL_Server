...
--EXEC HealthCheck.uspAlterCollunsDatetimeToDateOrDateTime2 @ObjectName = NULL,          -- varchar(200)
--                                                          
--                                                          @Efetivar = 1,          -- bit
--                                                          @Visualizar = 1,        -- bit
...
--BEGIN
    ...
    (
        ...
        WHERE S.cursor_name = 'cursor_CamposDateTime'
    )
       )
    BEGIN
    ...
    (
        SELECT 1
        FROM sys.syscursors AS S
        WHERE S.cursor_name = 'cursor_AlteraCampos'
    )
       )
    BEGIN
    ...
    AS (SELECT SchemaName = S.name,
        ...
        WHERE C.is_computed = 0
				...
				AND NOT EXISTS( SELECT * FROM sys.computed_columns AS CC
									WHERE CC.object_id =T.object_id AND 
						PATINDEX(CONCAT('%',C.name COLLATE DATABASE_DEFAULT,'%'),CC.definition COLLATE DATABASE_DEFAULT) >1 )
              AND (
                      @ObjectId IS NULL
   ...
   WHERE CDT.SchemaName LIKE 'HangFire%'
    ...
    BEGIN
        ...
        SET @Sql += CONCAT(SPACE(1), 'IF (EXISTS (   SELECT 1', SPACE(1));
        ...
        SET @Sql += CONCAT(SPACE(1), 'IF (EXISTS (   SELECT ', @Coluna, '');
        ...
        SET @SqlInsertTempTableDatetime2
            = CONCAT(
                        SPACE(2),
        ...
        SET @Sql += CONCAT(SPACE(1), 'ELSE');
        SET @Sql += CONCAT(SPACE(1), 'BEGIN');

        SET @SqlInsertTempTableDate
            = CONCAT(
                        SPACE(2),
                        'INSERT INTO #TabelasModificaveis  VALUES (',
                        CHAR(39),
        ...
        SET @Sql += @SqlInsertTempTableDate;

        SET @Sql += CONCAT(SPACE(1), 'END END');
        SET @Sql += CONCAT(SPACE(1), 'ELSE	 BEGIN', SPACE(1));
    ...
    AS (SELECT TM.SchemaName,
               ...
               DiminuicaoEmBytes = (((TM.Rows * 8))
                                    - (((TM.Rows * 8) - (TM.Rows * (IIF(TM.NovoTipo = 'DATE', 3, @tamanhoDateTime2)))))
                                   ),
               DiminuicaoEmPaginas = (((TM.Rows * 8)
                                       - ((TM.Rows * 8) - (TM.Rows * (IIF(TM.NovoTipo = 'DATE', 3, @tamanhoDateTime2))))
                                      )
                                      / 8060
                                     )
    ...
    SELECT R.SchemaName,
           ...
           ValorAtual = (SUM(R.TotalDePaginasOcupadasAntesAlteracaoParaColuna) OVER (PARTITION BY R.SchemaName, R.TableName))
                        - (SUM(R.DiminuicaoEmPaginas) OVER (PARTITION BY R.SchemaName, R.TableName))
    FROM Dados R;
    ...
    SELECT TM.SchemaName,
           ...
           IIF(CHARINDEX('getdate', DC.definition, 0) = 0,
               REPLACE(DC.definition, '(((1900)-(0))-(0))', CONCAT(CHAR(39), '1900-0-0', CHAR(39))),
               DC.definition)
    FROM #TabelasModificaveis AS TM
        ...
        BEGIN
            ...
            BEGIN TRY
                ...
                (
                    ...
                    WHERE DE.ObjectId = @Object_id_Alter
                ...
                IF (@HasError = 0)
                    ...
                    BEGIN
                        ...
                        FETCH NEXT FROM cursor_DeleteIndex INTO @DeleteIndex;
                            ...
                            (
                                ...
                                FROM sys.indexes AS I
                                    JOIN sys.index_columns AS IC
                                        ON I.object_id = IC.object_id
                                           AND I.index_id = IC.index_id
                                WHERE I.object_id = @Object_id_Alter
                ...
                IF (@HasError = 0)
                    ...
                    (
                        ...
                        WHERE GCD.object_id = @Object_id_Alter
                ...
                IF (@HasError = 0)
                    ...
                    (
                        ...
                        WHERE GCI.ObjectId = @Object_id_Alter
                ...
                IF (@HasError = 0)
                    ...
                    (
                        ...
                        WHERE GCI.object_id = @Object_id_Alter
                              AND GCI.Collum_Id = @column_id_Alter
                    )
                       )
                    BEGIN
