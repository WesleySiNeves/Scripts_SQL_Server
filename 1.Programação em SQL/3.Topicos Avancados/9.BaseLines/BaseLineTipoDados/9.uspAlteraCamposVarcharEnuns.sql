...
WHERE CV.Coluna LIKE '%UF%'
AND CV.Type ='varchar'  AND CV.max_length =2
...
SELECT * FROM #CamposVarchar AS CV
WHERE CV.Coluna LIKE '%Tipo%'
    ...
    WHERE KC.parent_object_id IN (
                                     SELECT TM.object_id FROM #TabelasModificaveisParaChar AS TM
                                 )
          AND KC.type <> 'PK';
    ...
    BEGIN
		...
		SELECT * FROM Processo.ConfiguracoesMacrorregioesCEPs AS CMCP
