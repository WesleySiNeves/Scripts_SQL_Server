...
------Computed Columns
-- ==================================================================
--Observação: Nesse caso  aqui a coluna FullName é rederizada em tempo de execução
-- ==================================================================
...
/*  Nesse caso aqui a coluna fica armazenada no banco
*/
...
--Existem 97147 mil registros
SELECT CustomerTransactions.PaymentMethodID,
       COUNT(*) AS [Quantidade por grupo],
	   TotalLinhas = SUM(COUNT(*)) OVER()
  FROM Sales.CustomerTransactions
 GROUP BY CustomerTransactions.PaymentMethodID;
GO

--E para Buscar 26637  mil registros ele usou o Seek
SELECT *
FROM   Sales.CustomerTransactions
...
INCLUDE ( OrderDate,ExpectedDeliveryDate) WITH(DROP_EXISTING =ON)
ON USERDATA;
GO
...
INCLUDE (FullName) WITH(DROP_EXISTING = ON)
ON USERDATA;
GO
...
CREATE NONCLUSTERED INDEX [Idx_CustomerTransactionsCustomerTransactionID]
...
DROP VIEW IF EXISTS Application.PeopleEmployeeStatus
...
CREATE NONCLUSTERED INDEX [InvoicesInvoiceDate]
...
/*
Falha da tentativa de inserção ou de atualização porque a vista de destino especifica WITH CHECK OPTION 
*/
------Modifying data in views with more than one table
DROP TABLE IF EXISTS Examples.GadgetType
...
DROP VIEW IF EXISTS Examples.GadgetExtension
...
DROP VIEW IF EXISTS Examples.InvoicesPartitioned
...
DROP VIEW IF EXISTS Sales.InvoiceCustomerInvoiceAggregates
GO
...
GO
*/
DROP INDEX IF EXISTS [CCX_Fact_Order] ON [Fact].[Order];
GO
...
CREATE NONCLUSTERED COLUMNSTORE INDEX [NCCX_Fact_Order] ON [Fact].[Order] ( 
       [Order Key] ,[City Key] ,[Customer Key] ,[Stock Item Key]
      ,[Order Date Key] ,[Picked Date Key] ,[Salesperson Key] ,[Picker Key]
      ,[WWI Order ID],[WWI Backorder ID],[Package]
      ,[Quantity],[Unit Price],[Tax Rate],[Total Excluding Tax]
      ,[Tax Amount],[Total Including Tax]);


/*
Tabela 'Customer'. Contagem de verificações 5, leituras lógicas 40, leituras físicas 0, leituras antecipadas 0, l
Tabela 'Date'. Contagem de verificações 5, leituras lógicas 79, leituras físicas 0, leituras antecipadas 0, leitu
Tabela 'Order'. Contagem de verificações 7, leituras lógicas 5908, leituras físicas 3, leituras antecipadas 5905,
Tabela 'Workfile'. Contagem de verificações 0, leituras lógicas 0, leituras físicas 0, leituras antecipadas 0, le
Tabela 'Worktable'. Contagem de verificações 0, leituras lógicas 0, leituras físicas 0, leituras antecipadas 0, l
*/
	  SET STATISTICS IO ON 
	  SET STATISTICS TIME ON  
...
(
	...
	UnitPrice,
	PickedQuantity
) WITH ( DROP_EXISTING =ON, COMPRESSION_DELAY = 5) ON USERDATA;
GO
...
DROP INDEX IF EXISTS NCCI_Orders ON Sales.Orders
...
DROP INDEX IF EXISTS [CColumnStore] ON [Fact].[SaleLimited]
...
SELECT TOP (100000) [City Key], [Customer Key],  [Bill To Customer Key], 
          [Stock Item Key], [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
          [WWI Invoice ID], [Description], [Package], [Quantity]
FROM Fact.Sale 
...
INSERT INTO [Fact].[SaleLimited] WITH (TABLOCK)  
     ([City Key], [Customer Key],  [Bill To Customer Key], [Stock Item Key],
      [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
      [WWI Invoice ID], [Description], [Package], [Quantity]) 
SELECT TOP (102400) [City Key], [Customer Key],  [Bill To Customer Key], 
          [Stock Item Key], [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
          [WWI Invoice ID], [Description], [Package], [Quantity]
FROM Fact.Sale 
...
INSERT INTO [Fact].[SaleLimited] WITH (TABLOCK)  
([City Key],	[Customer Key],
	         [Bill To Customer Key], [Stock Item Key],
	         [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
	         [WWI Invoice ID], [Description], [Package], [Quantity]) 
SELECT TOP (5000) [City Key],	[Customer Key],
	         [Bill To Customer Key], [Stock Item Key],
	         [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
	         [WWI Invoice ID], [Description], [Package], [Quantity]
FROM Fact.Sale; 
...
INSERT INTO [Fact].[SaleLimited] 
([City Key],	[Customer Key],
	         [Bill To Customer Key], [Stock Item Key],
	         [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
	         [WWI Invoice ID], [Description], [Package], [Quantity]) 
SELECT TOP (100000 ) [City Key],	[Customer Key],
	         [Bill To Customer Key], [Stock Item Key],
	         [Invoice Date Key], [Delivery Date Key],[Salesperson Key],
	         [WWI Invoice ID], [Description], [Package], [Quantity]
FROM Fact.Sale; 
