...
FROM (  SELECT plan_handle, query_plan
		...
		WHERE tp.query_plan.exist('//ColumnReference[@Database="[15.1-implanta]"]')=1
...
--CREATE UNIQUE NONCLUSTERED INDEX [UX_HangFire_Hash_Key_Field] ON [HangFire].[Hash] ([Key], [Field]) INCLUDE(Value) WITH(DROP_EXISTING =ON)
