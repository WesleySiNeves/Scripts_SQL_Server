...
CREATE TABLE #TabelasExcecao (Nome VARCHAR(128));

INSERT INTO #TabelasExcecao (Nome)
VALUES ('Logs'),('LogsDetalhes')
...
WHILE EXISTS ( SELECT   Id
    ...
    WHERE   p.index_id IN ( 0, 1 ) -- 0 heap table , 1 table with clustered index
			...
			AND p.rows > 100
			AND t.name NOT IN(SELECT TE.Nome COLLATE DATABASE_DEFAULT FROM #TabelasExcecao AS TE )
    GROUP BY s.[Name]
          , t.[name]
			...
			BEGIN
				...
				SET @Statement
					= 'UPDATE STATISTICS [' + @SchemaName + '].[' + @tableName + ']'
					  + IIF(CAST(@RowsInTable AS INT) < 10000, @FullScanSQL, ' WITH SAMPLE 50 PERCENT');

				SET @Status
					= 'Table ' + @TableNumber + ' of ' + @NumTables + ': Running ''' + @Statement + ''' (' + @RowsInTable
					  + ' rows)';
