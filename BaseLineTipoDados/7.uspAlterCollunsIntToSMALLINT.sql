EXEC HealthCheck.uspAlterCollunsIntToSMALLINT @Efetivar = 1,
                                              @Visualizar = 1,
                                              @ObjectName = NULL;
...
EXEC HealthCheck.uspIndexDesfrag @MostrarIndices = 0, -- bit
                                 @MinFrag = 10,           -- smallint
                                 @MinPageCount = 100,      -- smallint
                                 @Efetivar = 1        -- bit
...
BEGIN
    ...
    IF (OBJECT_ID('TEMPDB..#CamposPreferencialmenteSmallInt') IS NOT NULL)
        DROP TABLE #CamposPreferencialmenteSmallInt;


    CREATE TABLE #CamposPreferencialmenteSmallInt (Coluna VARCHAR(200));
    INSERT INTO #CamposPreferencialmenteSmallInt (
                                                 Coluna
                                                 )
    VALUES ('%Ano%'),
    ('%Exercicio%'),
    ...
    IF (OBJECT_ID('TEMPDB..#CamposSmallInt') IS NOT NULL)
        DROP TABLE #CamposSmallInt;

    CREATE TABLE #CamposSmallInt (
                                 SchemaName VARCHAR(128),
                                 TableName  VARCHAR(128),
                                 Coluna     VARCHAR(128),
                                 );
    ...
    INSERT INTO #CamposSmallInt (
                                SchemaName,
                                TableName,
                                Coluna
                                )
    VALUES ('Processo',            -- SchemaName - varchar(128)
            'ProcessosAndamentos', -- TableName - varchar(128)
            'Quantidade'           -- Coluna - varchar(128)
           ),
    ('Registro',            -- SchemaName - varchar(128)
     'FormacoesAcademicas', -- TableName - varchar(128)
     'Duracao'              -- Coluna - varchar(128)
    );
    ...
    FROM DadosTabela R
         ...
         (
         SELECT TPT.Coluna
         FROM #CamposPreferencialmenteSmallInt AS TPT
         ) Filtro ON R.Coluna LIKE Filtro.Coluna
         LEFT JOIN
         (
         SELECT TPT.Name
         FROM #CamposExcecoes AS TPT
         ) FiltroExc ON R.Coluna LIKE FiltroExc.Name
    WHERE [Type] = 'int'
          AND (
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN CATCH
                IF (@@TRANCOUNT > 0)
                BEGIN
                    PRINT 'ROLLBACK TRANSACTION Task';
    ...
    BEGIN
        ...
        BEGIN
            ...
            FROM #Reducao AS R
            ORDER BY
                R.TamanhoAtualParaColunaPaginas DESC;
