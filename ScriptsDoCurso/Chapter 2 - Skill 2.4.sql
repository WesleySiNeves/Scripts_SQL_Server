...
-- ==================================================================
--Observação:Antes de começar a demo deleta se existir
/*
 */
-- ==================================================================
IF (EXISTS (   SELECT *
                 FROM sys.tables AS T
                WHERE T.object_id = OBJECT_ID('dbo.Products')))
BEGIN
...
SELECT P.productid,
  ...
  FROM dbo.Products AS P;
  	...
  	-- ==================================================================
	--Observação: COmo incluimos a clausula HIDDEN ,quando fazemmos um select * na tabela
	--os campos temporais não são mostrados
	-- ==================================================================
...
SELECT CURRENT_TIMESTAMP; --2018-06-01 17:36:48.703
...
SELECT productid, supplierid, unitprice, validfrom, validto
FROM dbo.ProductsHistory
WHERE ProductsHistory.productid =6
ORDER BY ProductsHistory.supplierid,ProductsHistory.validfrom


-- modifying data in a transaction uses transaction start time (14:10:43.470)
BEGIN TRAN;

PRINT CAST(SYSUTCDATETIME() AS DATETIME2(3));  --2018-06-04 11:14:13.506

PRINT CAST(CURRENT_TIMESTAMP AS DATETIME2(3));  --2018-06-04 11:14:13.506
...
-- ==================================================================
--Observação: Veja que o tempo guardado na tabela  e o tempo inicial da transação
/*
 */
-- ==================================================================
...
SELECT productid, supplierid, unitprice,Products.validto
FROM dbo.Products FOR SYSTEM_TIME AS OF '20161101 14:07:55.000';


SELECT productid, supplierid, unitprice,Products.validto
FROM dbo.Products FOR SYSTEM_TIME AS OF '2016-11-01 14:10:43.469';


-- ==================================================================
--Observação: FOR SYSTEM_TIME AS OF
/* Essa clausula vale para a data acima executa o operador >
 */
-- ==================================================================
SELECT productid, supplierid, unitprice,Products.validto
FROM dbo.Products FOR SYSTEM_TIME AS OF '2016-11-01 14:10:43.469';
...
-- ==================================================================
--Observação:   FOR SYSTEM_TIME FROM @start TO @end 
/*  aqui executa ( Data > @start and Data <  @end )
 */
-- ==================================================================
SELECT productid, supplierid, unitprice, validfrom, validto
FROM dbo.Products
  FOR SYSTEM_TIME FROM '2016-11-01 14:07:26.263' TO '20161101 14:11:38.113'
WHERE productid = 9;


SELECT productid, supplierid, unitprice, validfrom, validto
FROM dbo.Products
  FOR SYSTEM_TIME FROM '2016-11-01 14:09:18.584' TO '2016-11-01 14:10:43.470'
WHERE Products.supplierid = 3;


SELECT productid, supplierid, unitprice, validfrom, validto
FROM dbo.Products
  WHERE CAST(Products.validfrom AS DATE)  ='2016-11-01'
AND Products.supplierid = 3;


SELECT productid, supplierid, unitprice, validfrom, validto
FROM dbo.Products
  FOR SYSTEM_TIME FROM '2016-11-01 14:01:01.001' TO '2016-11-01 14:09:18.001'
WHERE Products.supplierid = 3;
...
SELECT TOP 2
    cat.categoryname,
    produto.productid AS 'Produto.Nome',
    produto.unitprice AS 'Produto.Preco'
FROM Production.Categories cat
    JOIN Production.Products produto
        ON cat.categoryid = produto.categoryid
ORDER BY cat.categoryid
FOR JSON AUTO;

--WITHOUT_ARRAY_WRAPPER  Remove "[]"
-- Remove brackets
SELECT c.custid AS [Customer.Id], 
  c.companyname AS [Customer.Name], 
  o.orderid AS [Customer.Order.Id], 
