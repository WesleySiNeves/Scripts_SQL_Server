...
--BEGIN
    ...
    DECLARE @TamanhoTINYINT SMALLINT = 2;
	...
	VALUES ('HangFire' -- Name - varchar(200)
	       )
...
SELECT * FROM #CamposInt AS CI
    ...
    SET @Sql    += CONCAT('IF ((SELECT TRY_CAST(MAX(',@Coluna,') AS SMALLINT) FROM ',@SchemaName,'.',@TableName,') > 1 AND (SELECT TRY_CAST(MAX(',@Coluna,') AS SMALLINT) FROM ',@SchemaName,'.',@TableName,') < 5000 ) BEGIN ');
