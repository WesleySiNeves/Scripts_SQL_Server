...
SELECT [Database] = COALESCE(DB_NAME(DTDT.database_id),DB_NAME()),
       ...
       [Sess�o] = DES.session_id,
       [transaction_id] = DTDT.transaction_id,
       [Nivel Isolamento] =  CASE WHEN DES.transaction_isolation_level =0 THEN 'N�o Especificado'
								  WHEN DES.transaction_isolation_level =1 THEN 'Leitura N�o Confirmada'
								  WHEN DES.transaction_isolation_level =2 THEN 'Leitura Confirmada'
								  WHEN DES.transaction_isolation_level =3 THEN 'Repet�vel'
								  WHEN DES.transaction_isolation_level =4 THEN 'Serializ�vel'
								  WHEN DES.transaction_isolation_level =5 THEN 'Instant�neo' END,
	   
	   [Tempo da transa��o aberta] =CONVERT(TIME,DATEADD (ms, DATEDIFF(MILLISECOND, DTDT.database_transaction_begin_time, CURRENT_TIMESTAMP), 0)),
...
ORDER BY R.[Tempo da transa��o aberta]
