DECLARE @defaultTunningPerform SMALLINT = 500;
...
WITH Dados
  AS (SELECT dm_mid.object_id,
             ...
             NomeIndex = CAST('IX_' + CAST(OBJECT_SCHEMA_NAME(dm_mid.object_id, dm_mid.database_id) AS VARCHAR(400))
                              + OBJECT_NAME(dm_mid.object_id, dm_mid.database_id) + '_'
                              + REPLACE(
                                    REPLACE(REPLACE(ISNULL(dm_mid.equality_columns, ''), ', ', '_'), '[', ''), ']', '')
                              + CASE
                                     WHEN dm_mid.equality_columns IS NOT NULL
                                      AND dm_mid.inequality_columns IS NOT NULL THEN CAST('_' AS VARCHAR(1))
                                     ELSE CAST('' AS VARCHAR(1))END
                              + REPLACE(
                                    REPLACE(
                                        REPLACE(ISNULL(dm_mid.inequality_columns, CAST('' AS VARCHAR(1))), ', ', '_'),
                                        '[',
                                        CAST('' AS VARCHAR(1))),
                                    ']',
                                    CAST('' AS VARCHAR(1))) AS VARCHAR(128))COLLATE DATABASE_DEFAULT,
...
BEGIN
    ...
    WITH Resumo
      ...
      AS (SELECT R.object_id,
                 ...
                 ColunaIncluida = CAST(LTRIM(
                                           RTRIM(
                                               REPLACE(
                                                   REPLACE(REPLACE(R.ColunaIncluida, '[', CHAR(32)), ']', CHAR(32)),
                                                   CHAR(32),
                                                   ''))) AS VARCHAR(1000)),
                 R.Avg_Estimated_Impact,
                 R.magic_benefit_number
            FROM Resumo R)
    INSERT INTO #Candidates (object_id,
                             s,
                             o,
                             equality_columns,
                             inequality_columns,
    ...
    SELECT R.object_id,
     ...
     WHERE (   R.magic_benefit_number >= @defaultTunningPerform
         AND   R.Avg_Estimated_Impact >= @defaultTunningPerform
          OR   (R.magic_benefit_number >= @defaultTunningPerform * 10)
         AND   (   R.[write:read ratio] < 1 -- Maior que 1 BAD Index
             AND   R.potential_read_op > (@defaultTunningPerform / 20.0)));

    DELETE FROM #Candidates
     WHERE #Candidates.object_id IN ( SELECT FRI.object_id FROM #FirtResultIntermediate AS FRI );
    ...
    SELECT C.object_id,
           ...
           C.[write:read ratio]
      FROM Custo2 C
     WHERE (   C.TotalMagic_benefit_number > @defaultTunningPerform
         AND   C.TotalAvg_Estimated_Impact > @defaultTunningPerform)
        OR (   C.TotalMagic_benefit_number > (@defaultTunningPerform * 20)
         AND   (   C.[write:read ratio]    < 1 -- Maior que 1 is BAD Index
    ...
    SELECT ObjectId = FI.object_id,
           ...
           ExisteIndiceNaChave = (CASE
                                       WHEN EXISTS (   SELECT 1
                                                        ...
                                                        WHERE I.object_id                     = FI.object_id
                                                          ...
                                                          AND IC.key_ordinal                  = 1) THEN 1
                                       ELSE 0 END),
           ChavePertenceAOutroIndice = (CASE
                                             WHEN EXISTS (   SELECT 1
                                                               FROM sys.indexes AS I
                                                               JOIN sys.index_columns AS IC
                                                                 ON I.object_id           = IC.object_id
           ...
           CreateIndex = CONCAT(
			  ...
			  (',
                             FI.Chave,
                             ')' + ISNULL(' INCLUDE (' + FI.ColunaIncluida + ')', SPACE(0)))
      FROM Final FI;
