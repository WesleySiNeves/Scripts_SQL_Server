...
ALTER   PROCEDURE [HealthCheck].[uspIndexDesfrag]  
(  
...
BEGIN  
    ...
    BEGIN  
        ...
        UPDATE FRAG
		SET 
			FRAG.NewFillFact = CASE  
				-- Para índices com page splits extremos  
				WHEN FRAG.PageSpltForIndex > 20000 THEN  
					CASE 
						WHEN (FRAG.FillFact - @MinFillFactorCritical) > 70 THEN (FRAG.FillFact - @MinFillFactorCritical)
						ELSE 70 
					END
				...
				WHEN FRAG.AvgFragmentationInPercent >= 90 THEN  
					CASE 
						WHEN (FRAG.FillFact - @MinFillFactorHigh) > 75 THEN (FRAG.FillFact - @MinFillFactorHigh)
						ELSE 75 
					END
 
				-- Para fragmentação alta com índices grandes  
				WHEN FRAG.AvgFragmentationInPercent >= 70 AND FRAG.PageCount > 10000 THEN  
					CASE 
						WHEN (FRAG.FillFact - @MinFillFactorHigh) > 75 THEN (FRAG.FillFact - @MinFillFactorHigh)
						ELSE 75 
					END
				...
				WHEN FRAG.AvgFragmentationInPercent >= 50 THEN  
					CASE 
						WHEN (FRAG.FillFact - @MinFillFactorMedium) > 80 THEN (FRAG.FillFact - @MinFillFactorMedium)
						ELSE 80 
					END
				...
				WHEN FRAG.AvgFragmentationInPercent >= @MinFrag THEN  
					CASE 
						WHEN (FRAG.FillFact - @MinFillFactorLow) > 85 THEN (FRAG.FillFact - @MinFillFactorLow)
						ELSE 85 
					END
