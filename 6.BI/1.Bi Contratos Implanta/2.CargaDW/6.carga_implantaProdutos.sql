...
SELECT DISTINCT DescricaoProdutoSigam,
       CodigoProdutoSigam
...
BEGIN
    ...
    DECLARE @maxId INT =
            (
                SELECT ISNULL(MAX(NumeroProduto), 0) FROM Implanta.Produtos WHERE NumeroProduto < 99
            );
    ...
    SELECT CONVERT(
                      ...
                      RIGHT('00000000'
                            + CONVERT(
                                         VARCHAR(8),
                                         CONVERT(VARBINARY(4), ROW_NUMBER() OVER (ORDER BY Codigo_Modulo) + @maxId),
                                         2
                                     ), 8) +
                      ...
                      RIGHT('000000000000' + CAST(ROW_NUMBER() OVER (ORDER BY Codigo_Modulo) + @maxId AS VARCHAR(12)), 12)
                  ) AS IdProduto,
    ...
    THEN INSERT(
               ...
               VALUES
               (
                source.NumeroProduto,
                source.IdProduto,
                source.DescricaoProdutoSigam,
                source.CodigoProdutoSigam,
    ...
    THEN UPDATE SET
                DescricaoProdutoCigam = source.DescricaoProdutoSigam,
...
SELECT 
    'Produtos Implanta' AS Tabela,
    COUNT(*) AS TotalRegistros,
    SUM(CASE WHEN CodigoProdutoSigam IS NOT NULL THEN 1 ELSE 0 END) AS ComCodigoSigam,
    SUM(CASE WHEN CodigoProdutoSigam IS NULL THEN 1 ELSE 0 END) AS SemCodigoSigam
FROM Implanta.Produtos

UNION ALL

SELECT 
    'DimProdutos' AS Tabela,
    COUNT(*) AS TotalRegistros,
    SUM(CASE WHEN CodigoProdutoCigam IS NOT NULL THEN 1 ELSE 0 END) AS ComCodigoSigam,
    SUM(CASE WHEN CodigoProdutoCigam IS NULL THEN 1 ELSE 0 END) AS SemCodigoSigam
FROM Shared.DimProdutos;

-- Limpeza das tabelas temporárias
DROP TABLE IF EXISTS #DadosSigam;

PRINT 'Script de carga de produtos finalizado com sucesso!';