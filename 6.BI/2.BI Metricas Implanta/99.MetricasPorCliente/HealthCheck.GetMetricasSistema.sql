...
SELECT
    @Metricas = 'QuantidadeArquivosAnexos,TotalGB';
                        ...
                        (
                            ...
                            WHERE
                                   ...
                                   AND REPLACE(
                                                  REPLACE(
                                                             REPLACE(
                                                                        REPLACE(REPLACE(s.NomeSistema, '.NET', ''), ' ', ''),
                                                                        '&', ''
                                                                    ), 'ção', 'cao'
                                                         ), 'NET', ''
...
IF (@RecuperarInformacoesArquivosAnexos = 1)
    BEGIN
        ...
        UPDATE
            target
        SET
            target.CodSistema = '00000000-0000-0000-0000-000000000009'
        FROM
            #MetricasArquivosAnexos AS target
        WHERE
            target.NomeSchema IN (
            ...
            WHERE
                    NOT (
                            ...
                            AND m.NomeMetrica IN (
                                                     'DataUltimoAcesso', 'QtdAcessos', 'QtdAcessosNoAno', 'PossueLicenca'
                                                 )
                        )
            ORDER BY
    ...
    BEGIN
        ...
        SELECT
            ...
            (
                ...
                WHERE
                       CodSistema IN (
                                         '1', '3', '4', '5', '6', '7', '8', '9', '10', '11', '14', '15', '16', '17', '19',
                                         '21', '23', '24', '99'
                                     )
...
FROM (
    SELECT R.Cliente,
           ...
           END AS Valor
    FROM #Retorno R
	LEFT JOIN #Sistemas se
                            ON se.IdSistemaEspelhamento = R.CodSistema
    WHERE R.CodSistema IN (''1'', 
                           ''3'',
                           ''4'', 
                           ''5'',
                           ''6'', 
