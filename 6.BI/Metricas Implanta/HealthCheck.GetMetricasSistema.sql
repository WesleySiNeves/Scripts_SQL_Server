...
BEGIN
    ...
    SELECT s.CodSistema,
           ...
           (
               ...
               WHERE Modulo = 'Global'
                     ...
                     AND REPLACE(
                                    REPLACE(REPLACE(REPLACE(REPLACE(Nome, '.NET', ''), ' ', ''), '&', ''), 'ção', 'cao'),
                                    'NET',
                                    ''
    ...
    FROM Sistema.Sistemas s
        ...
        (
            ...
            FROM
            (
                SELECT stotal.IdSistema,
                       MAX(stotal.DataUltimoRequest) AS DataUltimoAcesso,
                       COUNT(1) AS QtdAcessos
                FROM Acesso.Sessoes stotal
                GROUP BY stotal.IdSistema
            ) AS AcessoGeral
                LEFT JOIN
                (
                    SELECT sinterno.IdSistema,
                           COUNT(1) QtdAcessosNoAno
                    FROM Acesso.Sessoes sinterno
                    WHERE YEAR(sinterno.DataInicio) = YEAR(GETDATE())
                    GROUP BY sinterno.IdSistema
                ) AcessoPorExercicio
    ...
    SELECT '00000000-0000-0000-0000-000000000099' AS CodSistema,
           IIF(
              (
                  SELECT TOP 1
                         1
                  FROM Sistema.Configuracoes AS C
                  WHERE C.Configuracao = 'LicencaCentroCustos'
           ...
           CASE
               WHEN @OrcamentoPorCentroCusto = 1
                    OR @UtilizaCentroCustoPagamento = 1 THEN
               (
                   SELECT MAX(ep.DataUltimoAcesso)
                   FROM #MetricasBase ep
                   WHERE ep.CodSistema = '00000000-0000-0000-0000-000000000001'
               )
               ELSE
                   NULL
           ...
           CASE
               ...
               (
                   SELECT QtdAcessos
                   FROM #MetricasBase
                   WHERE CodSistema = '00000000-0000-0000-0000-000000000001'
               )
               ELSE
                   0
           END AS QtdAcessos,
    ...
    WITH InfoSOnline
    AS (SELECT '00000000-0000-0000-0000-000000000018' AS CodSistema, --Servicos Online
               MAX(DataAcesso) AS DataUltimoAcesso,
               COUNT(1) AS QtdAcessos,
               (
                   SELECT COUNT(1) AS QtdAcessosNoAno
                   FROM Online.Acessos
                   WHERE YEAR(DataAcesso) = YEAR(GETDATE())
               ) AS QtdAcessosNoAno,
               (
                   SELECT ISNULL(IIF(Valor = 'true', 1, 0), 0)
                   FROM Sistema.Configuracoes
                   WHERE Configuracao = 'AcessoServicoOnlineSiscaf'
               ) AS PossueLicenca
        FROM Online.Acessos)
...
WHERE pc.Codigo LIKE ''[78].%'' AND YEAR(p.Data) = YEAR(GETDATE())) AS bit ),0)'   -- Script - varchar(max)
    ...
    (   '00000000-0000-0000-0000-000000000001', 'QtdPagamentosNoAno', -- Nome - varchar(200)
        'INT',                                                        -- TipoRetorno - varchar(30)
        'despesa.pagamentos', 'SELECT COUNT(1) FROM Despesa.Pagamentos s WHERE YEAR(s.DataPagamento) = YEAR(GETDATE())'),
    ...
    (   '00000000-0000-0000-0000-000000000009', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'Licitacao.Licitacoes', 'SELECT COUNT(1) FROM   Licitacao.Licitacoes WHERE  YEAR(Data)  = YEAR(GETDATE())'),
    --GestaoTCU   
    ...
    (   '00000000-0000-0000-0000-000000000010', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'TCU.RelatoriosTCU', 'SELECT COUNT(1) FROM   TCU.RelatoriosTCU WHERE  YEAR(DataGeracao)  = YEAR(GETDATE())'),
    ...
    (   '00000000-0000-0000-0000-000000000011', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'Registro.Registros', 'SELECT Count(1) FROM  Registro.Registros  WHERE YEAR(DataCriacao) = YEAR(GETDATE())'),
    --Sisdoc.NET
    ...
    (   '00000000-0000-0000-0000-000000000014', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'Documento.Documentos', 'SELECT Count(1) FROM  Documento.Documentos  WHERE YEAR(DataCriacao) = YEAR(GETDATE())'),
    --Processos.NET
    ...
    (   '00000000-0000-0000-0000-000000000023', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'Processo.Processos', 'SELECT Count(1) FROM Processo.Processos  WHERE YEAR(DataCriacao) = YEAR(GETDATE())'),
    --Fiscalização.NET
    ...
    (   '00000000-0000-0000-0000-000000000024', 'QtdRegistroAno', -- Nome - varchar(200)
        'INT',                                                    -- TipoRetorno - varchar(30)
        'Fiscalizacao.Visitas', 'SELECT Count(1) FROM Fiscalizacao.Visitas  WHERE YEAR(DataCriacao) = YEAR(GETDATE())');
    ...
    DECLARE @CodSistema UNIQUEIDENTIFIER,
            @Ordem TINYINT,
            @Nome VARCHAR(100),
    ...
    BEGIN
        ...
        FROM #MetricasArquivosAnexos AS target
            ...
            (
                SELECT DISTINCT
                       ...
                       st.Sistema,
                       st.NomeSchema
                FROM #TabelasPrincipaisSistema st
            ) AS regra
        ...
        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000007' -- 'ComprasContratos.NET'
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema IN ( 'Compra', 'Contratos' )
              AND target.CodSistema IS NULL;


        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000010' -- 'GestaoTCU.NET'
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema LIKE 'TCU%'
              AND target.CodSistema IS NULL;



        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000016' --'Portal Transparencia.NET'
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema LIKE '%Transparencia%'
              AND target.CodSistema IS NULL;
        ...
        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000001' --'Siscont.NET'
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema IN ( 'REINF', 'Despesa', 'Receita', 'Contabilidade', 'Orcamento' )
              AND target.CodSistema IS NULL;
        ...
        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000011' --Siscaf
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema IN ( 'AtivosBB', 'Financeiro', 'Plenaria', 'Requerimento', 'Serasa', 'Siscaf',
                                     'Online', 'Recadastramento'
                                   )
              AND target.CodSistema IS NULL;

        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000014' --Sisdoc
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema IN ( 'Formulario' )
              AND target.CodSistema IS NULL;


        UPDATE target
        SET target.CodSistema = '00000000-0000-0000-0000-000000000000'
        FROM #MetricasArquivosAnexos AS target
        WHERE target.NomeSchema IN ( 'Sistema', 'Cadastro', '' )
              AND target.CodSistema IS NULL;
        ...
        FROM #Metricas target
            JOIN
            (
                SELECT base.CodSistema,
                       info.NomeMetrica,
                       info.Valor
                FROM
                (SELECT DISTINCT CodSistema FROM #Metricas) base
                    JOIN
                    (
                        SELECT CodSistema,
                        ...
                        (
                            ...
                            (
                                Valor
                                FOR TipoMetrica IN (QuantidadeArquivosAnexos, [TotalGB])
                            ) AS UnpivotTable
                    ) info
    ...
    SELECT SUBSTRING(DB_NAME(), 0, CHARINDEX('.', DB_NAME())) AS Cliente,
           R.CodSistema,
           UPPER(ISNULL(s.Descricao, s.Nome)) AS Sistema,
           --R.TabelaConsultada AS [Tabela Principal],
           (
               SELECT rr.TabelaConsultada
               FROM #Metricas rr
               WHERE rr.CodSistema = R.CodSistema
    ...
    BEGIN
        ...
        (
            ...
            FROM #Retorno
            WHERE CodSistema IN ( '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000003',
                                  ...
                                  '00000000-0000-0000-0000-000000000019', '00000000-0000-0000-0000-000000000021',
